 ...
 * 
 */
public class AdminStaffPayrollSheet {
	private long id;
	 ...
	 */
	public AdminStaffPayrollSheet(long payrollId,long employeeId, float basicSalary,
			float overtime, float retirementPfEr, float allowanceOther,
			float grossSalary, float taxableIncome, float incomeTax,
			float retirementPfErDeduction, float retirementPfEe, float pension,
			float loanPfLoan, float other, float creditAssociation,
			float laborUnion, float totalDeduction, float netPay,
	 ...
	 * @param modificationDate
	 */
	public AdminStaffPayrollSheet(long id, long payrollId, long employeeId, float basicSalary,
			float overtime, float retirementPfEr, float allowanceOther,
			float grossSalary, float taxableIncome, float incomeTax,
			float retirementPfErDeduction, float retirementPfEe, float pension,
			float loanPfLoan, float other, float creditAssociation,
	...
	public void save() {
		try {
			String sqlStr = "insert into tbl_admin_staff_payroll_sheet values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public static void upate(AdminStaffPayrollSheet adminStaffPayrollSheet) {
		try {
			String sqlStr = "update tbl_admin_staff_payroll_sheet set payroll_id = ?, employee_id = ?," +
					"basic_salary = ?, overtime = ?, retirement_pf_er = ?, allowance_other = ?," +
					"gross_salary = ?, taxable_income = ?, income_tax = ?, retirement_pf_er_deduction = ?," +
					"retirement_pf_ee = ?, pension = ?, loan_pf_loan = ?, other = ?, credit_association = ?," +
					"labor_union = ?, total_deduction = ?, net_pay = ?, modified_by = ?, modification_date = ? where id = ?";
	...
	public static void delete(long id) {
		try {
			String sqlStr = "delete from tbl_admin_staff_payroll_sheet where id = ?";
			PreparedStatement pStmt = DBConnection.getPreparedStatement(sqlStr);
			pStmt.setLong(1, id);
			DBConnection.writeToDatabase(pStmt);
		} catch (Exception e) {
	...
	public static List<AdminStaffPayrollSheet> getAllAdminStaffPayrollSheets() {
		List<AdminStaffPayrollSheet> list = new ArrayList<AdminStaffPayrollSheet>();
		AdminStaffPayrollSheet adminStaffPayrollSheet = null;
		try {
			String query = "select * from tbl_admin_staff_payroll_sheet order by modification_date desc";
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				adminStaffPayrollSheet = new AdminStaffPayrollSheet(rSet.getLong("id"), rSet.getLong("payroll_id"), 
				...
				list.add(adminStaffPayrollSheet);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static List<AdminStaffPayrollSheet> getAllAdminStaffPayrollSheetsForThisPayroll(long payrollId){
		List<AdminStaffPayrollSheet> list = new ArrayList<AdminStaffPayrollSheet>();
		AdminStaffPayrollSheet adminStaffPayrollSheet = null;
		try {
			String query = "select tbl_admin_staff_payroll_sheet.*,tbl_employee.first_name from tbl_admin_staff_payroll_sheet,tbl_employee where payroll_id = "+payrollId+" and tbl_employee.id = " +
					"tbl_admin_staff_payroll_sheet.employee_id order by first_name, last_name asc";
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				adminStaffPayrollSheet = new AdminStaffPayrollSheet(rSet.getLong("tbl_admin_staff_payroll_sheet.id"), rSet.getLong("payroll_id"), 
						rSet.getLong("employee_id"), rSet.getFloat("basic_salary"), rSet.getFloat("overtime"), 
						rSet.getFloat("retirement_pf_er"), rSet.getFloat("allowance_other"), rSet.getFloat("gross_salary"), 
						rSet.getFloat("taxable_income"), rSet.getFloat("income_tax"), rSet.getFloat("retirement_pf_er_deduction"), 
						rSet.getFloat("retirement_pf_ee"), rSet.getFloat("pension"), rSet.getFloat("loan_pf_loan"), 
	...
	public static AdminStaffPayrollSheet getAdminStaffPayrollSheet(long id) {
		AdminStaffPayrollSheet adminStaffPayrollSheet = null;
		try {
			String query = "select * from tbl_admin_staff_payroll_sheet where id = "+id;
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				adminStaffPayrollSheet = new AdminStaffPayrollSheet(rSet.getLong("id"), rSet.getLong("payroll_id"), 
						rSet.getLong("employee_id"), rSet.getFloat("basic_salary"), rSet.getFloat("overtime"), 
		...
		return adminStaffPayrollSheet;
	}
	
	public static AdminStaffPayrollSheet getAllAdminStaffPayrollSheetsForThisPayrollAndEmployee(long payrollId,long employeeId){
		AdminStaffPayrollSheet adminStaffPayrollSheet = null;
		try {
			String query = "select * from tbl_admin_staff_payroll_sheet where payroll_id = "+payrollId+" and employee_id = "+employeeId;
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				adminStaffPayrollSheet = new AdminStaffPayrollSheet(rSet.getLong("id"), rSet.getLong("payroll_id"), 
						rSet.getLong("employee_id"), rSet.getFloat("basic_salary"), rSet.getFloat("overtime"), 
