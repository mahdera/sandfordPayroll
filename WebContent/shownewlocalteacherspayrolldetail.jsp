...
<%	
	...
	int ctr=1;
	String payrollName = "local_teachers_payroll_detail";
...
<table border="" width="100%" style="border: 1px solid #2779aa" rules="all">
	...
	<%
		...
		while(localTeacherEmployeeItr.hasNext()){
			...
			EmployeeSalary employeeSalary = EmployeeSalary.getEmployeeSalaryForThisEmployee(localTeacherEmployee.getId());
			if(employeeSalary != null){
				...
				AbsentDayInformation absentDayInformation = AbsentDayInformation.getAbsentDayInformationForEmployeeDuringThisYearAndMonth(localTeacherEmployee.getId(),year,payrollMonth);
				...
				float employeeOvertimeSum = EmployeeOvertime.getOvertimeSumForThisEmployeeBetweenDates(localTeacherEmployee.getId(),startDate,lastDate);			
				float employeeAllowanceSum = EmployeeAllowance.getEmployeeAllowanceSumForThisEmployee(localTeacherEmployee.getId());
							
				EmployeeDeduction providentFundERDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(),"Provident Fund - ER");
				float pfER = 0.0f;
				...
				EmployeeDeduction providentFundEEDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(),"Provident Fund - EE");
				float pfEE = 0.0f;
				...
				EmployeeDeduction pensionDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(),"Pension");
				float pension = 0.0f;
				...
				EmployeeDeduction loanDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(), "Loan");
				float loan = EmployeeLoan.getTotalMonthlyPaymentAmountSumForThisEmployee(localTeacherEmployee.getId());
				float advancePayment = 0.0f;
				
				advancePayment = Advance.getAllAdvancePaymentsForEmployeePaiedDuring(localTeacherEmployee.getId(),startDate,lastDate);
				
				EmployeeDeduction otherDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(), "Other");
				float other = 0.0f;
				...
				EmployeeDeduction creditAssociationDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(), "Credit Association");
				...
				EmployeeDeduction laborUnionDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(localTeacherEmployee.getId(), "Labor Union");
				...
				%>
					<tr>
						<td><%=ctr++ %></td>
						<td><%=fullName %><input type="hidden" name="<%=employeeIdControlName %>" id="<%=employeeIdControlName %>" value="<%=localTeacherEmployee.getId() %>"/></td>
				...
				//now do the total calculation in here...
				 ...
				 totalNetPay += netPay;
			}//end employeeSalary null checker if condition...
		}//end while loop
	%>
	<tr style="background:#eee;font-weight:bolder">
		<td></td>
...
%>
		<p class="msg warning">No local teacher staff is mapped by the administrator. Please finish mapping payroll employee types first!</p>
<%
	}
%>
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#btnsave').click(function(){
			...
			$.ajax({
			    url: 'savelocalteacherspayrollsheet.jsp',		
			    data: dataString,
			    type:'POST',
