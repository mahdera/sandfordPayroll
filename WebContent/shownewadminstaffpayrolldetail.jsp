...
<%	
	...
	String payrollName = "admin_staff_payroll_detail";
...
<table border="" width="100%" style="border: 1px solid #2779aa" rules="all">
	...
	<%
		...
		while(adminEmployeeItr.hasNext()){
			...
			EmployeeSalary employeeSalary = EmployeeSalary.getEmployeeSalaryForThisEmployee(adminEmployee.getId());
			if(employeeSalary != null){
				...
				AbsentDayInformation absentDayInformation = AbsentDayInformation.getAbsentDayInformationForEmployeeDuringThisYearAndMonth(adminEmployee.getId(),year,payrollMonth);
				...
				float employeeOvertimeSum = EmployeeOvertime.getOvertimeSumForThisEmployeeBetweenDates(adminEmployee.getId(),startDate,lastDate);
				
				float employeeAllowanceSum = EmployeeAllowance.getEmployeeAllowanceSumForThisEmployee(adminEmployee.getId());
						
				EmployeeDeduction providentFundERDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(),"Provident Fund - ER");
				float pfER = 0.0f;
				...
				EmployeeDeduction providentFundEEDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(),"Provident Fund - EE");
				float pfEE = 0.0f;
				...
				EmployeeDeduction pensionDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(),"Pension");
				float pension = 0.0f;
				...
				float loan = EmployeeLoan.getTotalMonthlyPaymentAmountSumForThisEmployee(adminEmployee.getId());			
				
				EmployeeDeduction otherDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(), "Other");
				float other = 0.0f;
				...
				EmployeeDeduction creditAssociationDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(), "Credit Association");
				...
				EmployeeDeduction laborUnionDeduction = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(), "Labor Union");
				float laborUnion = 0.0f;
				if(laborUnionDeduction != null){
					laborUnion = salary.getAmount() * laborUnionDeduction.getDeductionAmount();
				}
				
				EmployeeDeduction pfLoanDeducation = EmployeeDeduction.getEmployeeDeductionForThisEmployeeAndType(adminEmployee.getId(), "PF Loan");
				//float pfLoan = 0.0f;
				float advancePayment = 0.0f;		
				advancePayment = Advance.getAllAdvancePaymentsForEmployeePaiedDuring(adminEmployee.getId(),startDate,lastDate);
				...
				%>
					<tr>
						<td><%=ctr++ %></td>
						<td><%=fullName %><input type="hidden" name="<%=employeeIdControlName %>" id="<%=employeeIdControlName %>" value="<%=adminEmployee.getId() %>"/></td>
...
%>
		<p class="msg warning">No administrative staff is mapped by the administrator. Please finish mapping payroll employee types first!</p>
<%
	}
%>
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#btnsave').click(function(){
			...
			$.ajax({
			    url: 'saveadminstaffpayrollsheet.jsp',		
			    data: dataString,
			    type:'POST',
